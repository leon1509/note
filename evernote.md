...
### 技术备忘
````
solr, hbase, hive, kylin, storm, kafka, zookeeper, spark, jenkins

nohup websocketd --port 3333 --staticdir=. tail -f /project/ebag/logs/error.log &

ppo： $(curl -s4 icanhazip.com)
xagc： NODEIP=$(curl -s4 api.ipify.org)
totalk=$(awk '/^MemTotal:/{print $2}' /proc/meminfo) ;    #获取内存大小
             if [ $totalk -lt 2097152 ] ; then   #如果内存小于2G

              # 读取控制台输入，赋值给变量masternodekey
              read -e -p "Please enter your masternode key and press ENTER: " masternodekey

redis lua 集群模式代码：https://github.com/steve0511/resty-redis-cluster

==== linux下查找某目录及其子目录下包含某字符的文件，并列出所在行：===
# grep -Rn -i "7207" *
...
scp -vrC root@172.31.13.7:/app/data .  带目录复制
scp -vrC xmrig-2.6.0-beta2 root@101.132.26.149:/root  复制目录到远程服务器
scp root@172.31.13.5:/project/report/lua/api/lua/api/school.lua lua/api

centos7下openjdk默认安装目录：/usr/java/jdk1.8.0_102
...
例：
java -jar -Dserver.port=10001 -Dlogging.file=/var/logs/report/admin.log report-eval-admin-1.0.jar &
java -jar -Dserver.port=10002 -Dlogging.file=/var/logs/report/web.log report-eval-web-1.0.jar &
java -jar -Dserver.port=10003 -Dlogging.file=/var/logs/report-api.log report-api-1.0.jar&

java -jar report-web-1.1.jar --spring.config.location=report-web.yml

java -Xms256m -Xmx512m -jar -Dlogging.file=/var/logs/ebag-school.log LK-school-1.0.jar --spring.config.location=school-prod.yml &
java -Xms256m -Xmx512m -jar -Dlogging.file=/var/logs/ebag-admin.log LK-admin-1.0.jar --spring.config.location=admin-prod.yml
java -Xms256m -Xmx512m -jar -Dlogging.file=/var/logs/ebag-teacher.log LK-teacher-1.0.jar --spring.config.location=teacher-prod.yml &


java -Xms256m -Xmx512m -Dserver.port=9000 -jar Ali-OSSService-1.0.jar --spring.config.location=Ali-OSSService.yml &   OSS接口服务
java -Xms256m -Xmx512m -Dserver.port=9001 -jar pay-Alipay-1.0.jar --spring.config.location=pay-Alipay.yml &    支付宝服务
java -Xms256m -Xmx512m -Dserver.port=9002 -jar NIM-Holder-1.0.jar --spring.config.location=NIM-Holder.yml &    NIM对内服务
java -Xms256m -Xmx512m -Dserver.port=9003 -jar NIM-Server-1.0.jar --spring.config.location=NIM-Server.yml &    NIM对外服务
java -Xms256m -Xmx512m -Dserver.port=9004 -jar Mq-Producer-Agent-1.0.jar --spring.config.location=Mq-Producer-Agent.yml &   消息队列生产者服务
java -Xms256m -Xmx512m -Dserver.port=9005 -jar Mq-Consumer-Agent-1.0.jar --spring.config.location=Mq-Consumer-Agent.yml &   消息队列消费者服务
java -Xms256m -Xmx512m -Dserver.port=9006 -jar Assignment-Service-1.0.jar --spring.config.location=Mq-Consumer-Agent.yml &  作业接口服务

java -Xms256m -Xmx512m -Dserver.port=9000 -jar LK-school-1.0.jar --spring.config.location=school-prod.yml
java -Xms256m -Xmx512m -Dserver.port=9002 -jar LK-admin-1.0.jar --spring.config.location=admin-prod.yml

CURL 乱码（https://zhidao.baidu.com/question/548117332.html）：
先抓下来看看编码, 估计是 gb2312, 然后转换
curl www.foo.com | iconv -f gb2312 -t utf-8

nginx -p `pwd` -c config/nginx.conf [-s reload/quit]
nginx -p /project/report/lua/mobile -c config/nginx.conf [-s reload/quit]
...
Openresty 定时任务参考：
    1. https://groups.google.com/forum/#!msg/OpenResty/0aEE-n2tUWE/Wo6cb4UMBwAJ
    2. http://www.shixinke.com/openresty/openresty-ngx-timer

停止某程序（kinaba）进程 :
    1. fuser -n tcp 5601    #查找5601端口的进程id
    2. kill -9 进程id
...
linux(centOS7)下word文件转pdf :
参考： http://www.cnblogs.com/xcp19870712/p/4760842.html
yum install libreoffice.x86_64
yum install unoconv
soffice --accept="socket,host=127.0.0.1,port=8100;urp;"-display :1&
unoconv -f pdf -o 1.pdf 1.docx
注意：首次转换会出现监视器选择，后续直接转换即可


============ 查找表users中姓名重复的数据 ================
SELECT `account`,`user_name`,`mobile`,`name`,count(`name`) as count FROM `users` GROUP BY `name` HAVING count(`name`)>1 ORDER BY count DESC;

x.lua:
    package.path="/xxx/xxx/?.lua"
    local xx = require "xxx"

bootstrap 固定列宽：
    <td style="word-wrap:break-word;width:720px;">
````
